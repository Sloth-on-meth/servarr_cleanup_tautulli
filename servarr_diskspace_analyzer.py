...
class ServarrTautulliAnalyzer:
    def __init__(
        ...
        if mode.lower() == "radarr":
            print(
                "Radarr mode is temporarily disabled as it's still under development."
            )
    ...
    def debug_request(
        self, name: str, url: str, params: dict, status_code: int, data: Any
    ) -> None:
    ...
    async def get_tautulli_library_sections(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            async with self.session.get(url, params=params) as response:
                ...
                self.debug_request(
                    "Get Tautulli Libraries", url, params, response.status, data
                )

                if data.get("response", {}).get("result") != "success":
                    print(
                        f"Error getting library sections from Tautulli: {data.get('response', {}).get('message')}"
                    )
                    return []
    ...
    async def generate_report(self, limit: int = None, months: int = 2) -> None:
        ...
        report_file = os.path.join(
            self.report_path, f"unwatched_{self.mode}_{timestamp}.json"
        )
        ...
        html_report_file = os.path.join(
            self.report_path, f"unwatched_{self.mode}_{timestamp}.html"
        )
    ...
    def generate_html_report(
        self, unwatched_items: List[Dict], file_path: str, months: int
    ) -> None:
...
async def main_async():
    parser = argparse.ArgumentParser(
        description="Analyze Sonarr/Radarr library and check Tautulli watch history."
    )
    parser.add_argument(
        "-c", "--config", default="config.ini", help="Path to config file"
    )
    parser.add_argument(
        "-l", "--limit", type=int, default=None, help="Limit to top N series by size"
    )
    parser.add_argument(
        "-m",
        "--months",
        type=int,
        default=2,
        help="Check if watched in the past N months",
    )
    parser.add_argument(
    ...
    analyzer = ServarrTautulliAnalyzer(
